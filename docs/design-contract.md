...
Toasts carry transient administrative feedback: a save that went out, a copy that landed, a background task that finished. A condition that persists until someone acts on it belongs in a `Banner` or a tone-marked `Card`, which stay on screen until the panel removes them, because a toast times out on the info and success tones and the queue holds five cards at once. A warning or danger toast is sticky and is never dropped for a lesser arrival, but a surface an operator has to come back to is still not a notification.
...
`PanelRoot` and `ThemeProvider` take `defaultTheme`, the choice a document with no stored preference starts on, for a panel that expects night use. It writes nothing, and it yields to a stored preference and to whichever mounted panel seeded the document first. There is deliberately no controlled `theme` prop: the preference is one document-wide value shared by every panel in the Admin page, so a panel holding it in its own state would fight the panel beside it.

### Where the theme selector goes

A Signal K Admin configuration panel puts the theme selector at the foot: `themeToggle="end"` on `PanelShell`. The prop already defaults to `"end"`, so writing it out changes nothing today; it is written out because a call site that records the decision reads as deliberate and survives a later change of default, and because a panel passing `"between"` today has to move to `"end"` anyway. Three reasons for the placement itself, in order of weight.

The selector is chrome over the panel, not one of its tasks. An operator opens a configuration panel to change a setting, and the first tab stop should be the first setting rather than a display preference. `"between"` on a panel that passes no `title` used to lead the panel for exactly that reason, which is why the shell now resolves it to `"end"`.

The preference is shared, as the paragraph above records, so the panel is not the only place to set it. A selector at the foot is reachable in the panel that happens to be open, not the sole route to a theme.

Consistency across panels is worth more than any single panel's optimum. An operator moving between plugin panels should find the selector in the same place each time, and a panel that positions it differently teaches them nothing except that the position is unreliable.

Two consequences follow, and both have caught a consumer:

- A panel that passes a `title` changes what `"between"` means, because a title gives it something to sit between. A panel wanting the selector at the foot must say `"end"` rather than rely on the resolution for a panel with no title, or adding a title later silently moves the selector to the middle of the panel.
- A panel rendering its own `ThemeToggle` somewhere else, with `themeToggle="none"` on the shell, is opting out of this convention. That is supported and occasionally right, but it is a deliberate choice to record at the call site rather than a default to drift into.

There is currently no supported placement that leads a panel with no title. If one is needed, it belongs in the library as a new `PanelShellThemeToggle` value rather than as `themeToggle="none"` plus a hand-placed toggle in each consumer.
...
- `--snui-color-row-selected-hover` (a selected `DataGrid` row under the pointer)
...
- `--snui-field-inline-label-min` (the inline field's label column)
- `--snui-grid-track-min` (the minimum track of a wrapping option grid)
- `--snui-action-bar-surface` (the sticky action bar's own fill)
- `--snui-content-width-standard`
- `--snui-content-width-wide`
- `--snui-focus-ring`
- `--snui-color-focus-ring-band` (the inner band of that ring, defaulting to the flat surface; the `dialog`, `popover`, and `toast` modules remap it to the raised surface so the band matches the fill behind the control)
...
- Single-choice segmented controls use radio-group semantics and roving focus. All four arrow keys move in either orientation, the way a radio group does, and each move selects as it goes; Home and End reach the ends; and holding Ctrl, or Command on macOS, moves focus without changing the selection.
- Horizontal segmented-control arrows follow document direction, read from the computed style rather than from a `:dir()` selector, and collapsible carets mirror in right-to-left layouts.
...
- High-contrast requests through `prefers-contrast: more` strengthen control borders to the text color, widen focus outlines to 3 pixels, and raise muted and disabled text toward the text color, with disabled text kept one step below the rest so a blocked control still reads as blocked.
...
- A role that announces on its own, `alert`, `log`, or `status`, does not also carry `aria-live`, because the pairing double speaks on some screen readers. Any other role, a landmark or a grouping for example, keeps the live mode the caller requested.
...
- Toasts stay reachable under a modal: the notifications host is a React Aria top layer, so it is neither hidden nor inert while a dialog is open, and F6 moves focus into the notifications region and back to where it was. F6 is answered only while focus is inside the owning panel, so a second panel and the Signal K Admin chrome keep the key, and F6 and Shift+F6 both toggle. Dismissing a toast moves focus to the card that takes its slot, else to the newest remaining toast, else back to the control that had it, never to the document body.
...
```css
@scope (.snui-root[data-snui-version="0.11.0"])
...
A module is not a promise that its component needs `PanelRoot`. The overlay modules above (`dialog`, `menu`, `popover`, and `toast`) throw outside one because they have nowhere to portal, as the overlay section below records; an in-flow control installs its module when it has an owning root and renders unstyled without one, exactly as it did while its rules traveled in the root sheet. `VisuallyHidden` and `LiveRegion` are the exception worth stating: the rule that hides them lives in the root sheet, so outside a `PanelRoot` their text is rendered visibly rather than hidden.
...
Dialogs size against the visual viewport and safe-area insets, including the narrow-panel bottom-sheet layout. `Dialog` writes the visible viewport height to `--snui-visual-viewport-height` and sizes itself from it, so an on-screen keyboard shrinks the dialog rather than pushing its actions behind the keyboard, and the dialog body is what scrolls: the title and the actions stay in view on a short landscape viewport. Anchored overlays are the exception to the inset rule: React Aria positions `Menu` and `Popover` against a fixed container padding, so they do not honor safe-area insets the way the scrim, the panel content, and the toast host do. The panel content itself keeps its horizontal padding at or above the left and right safe-area insets, so text in a notched or rounded viewport stays clear of the hardware edge. Every mounted toast region in one panel shares a single panel-owned host. That host tracks the intersection of the panel and visual viewport, respects safe-area insets, keeps independent queues from overlapping, and is removed after the final region unmounts. Toast exit removal follows the transition end, with a token-derived fallback timer for engines that omit the event and an immediate path for reduced motion.
...
Desktop controls have a compact 40-pixel minimum height. A device with any coarse pointer uses 44 pixels, and `--snui-space-2` rises to 0.75rem there, so adjacent actions separate as well as grow. Square and icon-only targets meet the same floor in both width and height, and a compact button carries that floor as a minimum width, so a button holding a single glyph is at least square rather than sized to the glyph. A segmented option carries it the same way, so a one-character option is at least square. A selectable `DataGrid` row is a target too, because pressing one changes the selection, so it holds the floor at default density in both the plain and the virtualized layouts. The range thumb opts out of native rendering, so it scales with this contract through `--snui-range-thumb-size` rather than relying on the user-agent target-size exception.
...
- The package's `/composites`, `/data-grid`, `/format`, `/forms`, and `/overlays` entry points are supported public import paths, and so are `/tokens.css`, `/federation`, and `/package.json`, the last two serving build tooling rather than the browser. The package root is the supported entry point for lightweight panel, layout, field, feedback, theme, compatibility, and formatting primitives. APIs assigned to a focused entry point are not also exported from the root, except `/format`, which republishes root helpers so a worker or a Node script can reach them without React in the graph.
