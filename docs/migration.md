...
- `CollapsibleSection` gains `leading` (content before the heading, outside the toggle), `variant="embedded"` (no chrome, for nesting in a `Card`), and `landmark`. `Card` gains `density="flush"` and `tone`, so local CSS that doubled selectors to remove padding or paint an accent border can go. Local status-dot CSS for chips can go too: `<StatusIndicator size="compact" tone={...}>Label</StatusIndicator>` shows the dot and glyph and keeps the label for assistive technology.
